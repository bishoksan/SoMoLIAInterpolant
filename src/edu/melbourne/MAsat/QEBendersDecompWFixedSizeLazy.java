 ...
 * @author kafle
 */
public class QEBendersDecompWFixedSizeLazy {
...
//        Util.print(PRINT_FLAG, "adding constraints gurobi");
        ...
        //the next two while loops are replacement for the following for loop
...
//        }
        int index = 0;
        while (index < currentAssumptions.length) {
            String id = currentAssumptions[index];
            boolean isTrueFormula = transformedFormulas.get(id).getOriginalFormula().isTrue();
            if (!isTrueFormula) {
                BoolExpr z3Formula = addConstraintToGrbSolver(id, grbModel, env, grbInterface, context, z3Interface, disjModel, refinementCount);
                if (!z3Formula.isTrue()) {
...
//            Util.print(PRINT_FLAG, "is SAT================================================");
//            System.out.println("is SAT================================================");
            //add delta free expressions
            qfLIAFormula = collectDeltaFreeFormulasFromCurrentAssumption(currentAssumptions, context, z3Interface);
            if (!qfLIAFormula.isTrue()) {
                formulaUnderCurrentAss = context.mkAnd(formulaUnderCurrentAss, qfLIAFormula);
            }
            //eliminate quantifiers
//            System.out.println("about to apply QE: " );
//            Util.print(PRINT_FLAG, "removing quantifiers ");
//            System.out.println("tpqe " + formulaUnderCurrentAss);
...
//            System.out.println("qf " + qfFormulaCurrentDecomposition);
//            Util.print(PRINT_FLAG, "quantifier removed");
//disjoin qf formulas
...
//            printCoreConstrants(unsatCore, context, disjModel);
...
//            System.out.println("refinement unsat "+refinementCount);
    ...
    BoolExpr collectDeltaFreeFormulasFromCurrentAssumption(String[] currentAssumption, Context context, Z3Interface z3Interface) {
    ...
    public BoolExpr addConstraintToGrbSolver(String formulaId, GRBModel grbModel, GRBEnv env, GurobiInterface grbInterface, Context context, Z3Interface z3Interface, Model deltaModel, int refinementCount) {
        ...
        if (deltaValuesModified && alreadyInSolver && !tf.isPrecise()) {
...
//        System.out.println("blocked models " + notCurrentDeltas);
        disjSolver.add(notCurrentDeltas);
        deltaValuesModified = true;
    }
...
//        acc = (BoolExpr) ctx.mkNot(acc).simplify();//put not
//        BoolExpr notCurrentDeltas = ctx.mkAnd(cutAssumptions, acc);
//        disjSolver.add(notCurrentDeltas);
//        deltaValuesModified = true;
//    }
...
//        }
//        cutAssumptions = assumptionFormula;
//        System.out.println("cut ass " + assumptionFormula);
//        Util.print(PRINT_FLAG, "converting assumptions to array");
//        return assumptions.toArray(new String[assumptions.size()]);
//    }
...
//            System.out.println("original formula " + bExpr);
            ...
            //is BV formula
//            Util.print(PRINT_FLAG, "preprocessed bv expr: " + bExpr);
...
//            System.out.println("nnf expr "+bExpr);
...
//                System.out.println("calling collect constraints");
    ...
    void printCoreConstrants(String[] unsatcore, Context ctx, Model deltaModel) {
        ...
        for (int i = 0; i < unsatcoreO.length; i++) {
            tf = transformedFormulas.get(unsatcoreO[i]);
    ...
    void printIdentifiedFormulas() {
        TransformedFormula tf;
        for (String i : transformedFormulas.keySet()) {
            tf = transformedFormulas.get(i);
