 ...
 * @author kafle
 */
public class QEBendersDecompWFixedSize {
...
//            System.out.println("transformed "+coreId);
        ...
        cutExpr = context.mkImplies(cutAssumptions, cutExpr);
...
//            System.out.println("refinement unsat "+refinementCount);
    ...
    public BoolExpr addConstraintToGrbSolver(String formulaId, GRBModel grbModel, GRBEnv env, GurobiInterface grbInterface, Context context, Z3Interface z3Interface, Model deltaModel, int refinementCount) {
        ...
        if (deltaValuesModified && alreadyInSolver && !tf.isPrecise()) { //delta model will not change and the constraint can stay in the solver
            for (int i = 0; i < nrConstraints; i++) {
        ...
        if (tf.isPrecise()) {
            z3Formula = tf.getArithFormulaWoDeltas(); //this is delta free formula needed for qe elimination
            if (z3Formula == null) { //no delta was introduced
                z3Formula = context.mkTrue();
                if (simplifiedFormula != null && simplifiedFormula.size() > 0) {
                    z3Formula = simplifiedFormula.get(0);
                }
                if (simplifiedFormula != null) {
                    for (int i = 1; i < simplifiedFormula.size(); i++) {
                        BoolExpr be = simplifiedFormula.get(i);
...
//TODO: remove quantified variables which are not in the formula
    ...
    public BoolExpr qeMAFromFormula(BoolExpr bExpr, Context context) {
        try {
            bExpr = preProcessQBVFormula(bExpr, context);
            Z3Interface z3Interface = new Z3Interface();
...
//            System.out.println("arith for "+arithFormulas.get(0).formula);
    ...
    public TransformedFormula getTransFormulaWithBothDeltas(IntExpr delta0, IntExpr delta1, String id, BoolExpr maExpr, BoolExpr varBounds, int sortSize, TransformedFormula tf, ArrayList<ArithFormula> arithFormulas, BoolExpr simplifiedAaExpr,
...
//            System.out.println("preprocessed expr ================= "+ preProcessedExpr);
        ...
        BoolExpr fullLiaExr = context.mkAnd(varBounds, deltaBounds);
        BoolExpr deltaFreeExpr = getDeltaFreeExpression(deltas[0], deltas[1], fullLiaExr, context, z3Interface);
        tf.setArithFormulaWoDeltas(deltaFreeExpr);
        tf.setFormula(arithFormulas);
        tf.setFormulaSize(5);
        return tf;
    }
...
//                System.out.println("preproc expr  "+preProcessedExpr.isTrue());
        ...
        BoolExpr deltaFreeExpr = getDeltaFreeExpression(rightDelta, fullLiaExr, context, z3Interface);
        tf.setArithFormulaWoDeltas(deltaFreeExpr);
        tf.setFormula(arithFormulas);
        tf.setFormulaSize(3);
        return tf;
...
//                System.out.println("preproc expr  "+preProcessedExpr.isTrue());
        ...
        BoolExpr fullLiaExr = context.mkAnd(moduloExpr, deltaBounds);
        BoolExpr deltaFreeExpr = getDeltaFreeExpression(leftDelta, fullLiaExr, context, z3Interface);
        tf.setArithFormulaWoDeltas(deltaFreeExpr);
        tf.setFormula(arithFormulas);
        tf.setFormulaSize(3);
        return tf;
    ...
    public BoolExpr getDeltaFreeExpression(IntExpr delta, BoolExpr fullLiaExr, Context ctx, Z3Interface z3Interface) {
    ...
    public BoolExpr getDeltaFreeExpression(IntExpr delta1, IntExpr delta2, BoolExpr fullLiaExr, Context ctx, Z3Interface z3Interface) {
