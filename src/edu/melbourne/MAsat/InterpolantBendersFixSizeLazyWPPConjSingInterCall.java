 ...
 * @author kafle
 */
public class InterpolantBendersFixSizeLazyWPPConjSingInterCall {
...
//returns assumptions in (previous-current)
    ...
    public BoolExpr[] getInterpolantFromCoreAndCutForAandB(Z3Interface z3Interface, InterpolationContext context, GRBEnv env, GurobiInterface grbInterface, Model modelA, Model modelB, GRBModel grbModelAB) {
        ...
        BoolExpr falseExpr = context.mkFalse();
        ...
        BoolExpr notB = context.mkNot(muB);
         if (countA == 0) { //b_j is inconsistent
            return new BoolExpr[]{falseExpr, falseExpr, notA, notB};
        }
        if (countB == 0) { //a_i is inconsistent
            return new BoolExpr[]{falseExpr, falseExpr, notA, notB};
        }
        return new BoolExpr[]{formulaA, formulaB, notA, notB};
    }
    ...
    public BoolExpr computeInterpolantTransformedFormula(Z3Interface z3Interface, InterpolationContext context, GRBEnv env, GurobiInterface grbInterface, Solver solverA, Solver solverB, GRBModel grbModelA, GRBModel grbModelB, GRBModel grbModelAB) {
        ...
        //partA and partB collect disjoint a_i and b_j
        BoolExpr partA = context.mkFalse();
        BoolExpr partB = context.mkFalse();
        Model modelA;
...
//                    System.out.println("B formula " + formulaUnderCurrentAssB);
                    ...
                    if (!LIAWitnessB.isSatisfiable()) {
                        ...
                        if (partialInterpolantAndCut == null) { //just to account for the fact that Gurobi may return satisfiable over reals even though it is unsat over integer
                            partA = context.mkOr(partA, formulaUnderCurrentAssA);
                            partB = context.mkOr(partB, formulaUnderCurrentAssB);
                            ...
                            //partialInterpolantAndCut={partA, partB, notA, notB}
                            partA = context.mkOr(partA, partialInterpolantAndCut[0]);
                            partB = context.mkOr(partB, partialInterpolantAndCut[1]);
                            solverbInnerLoop.add(partialInterpolantAndCut[3]); // not the current mu_b 
                            notMuA = partialInterpolantAndCut[2];
                            solverA.add(context.mkOr(notMuA, partialInterpolantAndCut[3])); // not the conjunction mu_a and mu_b  
                        }
                    }
                }
                solverA.add(notMuA);
        ...
        BoolExpr interpolant = getInterpolant(partA, partB, context, z3Interface);
        if (interpolant == null) {
