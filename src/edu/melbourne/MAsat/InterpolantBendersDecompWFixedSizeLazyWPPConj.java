 ...
 * @author kafle
 */
public class InterpolantBendersDecompWFixedSizeLazyWPPConj {
...
//            System.out.println("=================== ma_sat =========== ");
    ...
    public ArithExpr genAExprFromBvExpr(int exprWidth, Context ctx, String key, ArithExpr e, GRBModel grbModel, GurobiInterface grbInterface) {//number must have a fixed interpretation
        if (e.isIntNum()) { //z3 treats (- 8) as unary minus not as integer but -8 (without space) as integer
...
//    }
    ...
    public BoolExpr genAinEqFromAtomicBvinEq(Context ctx, String key, Expr e, Z3Interface z3Interface, GRBModel gRBModel, GurobiInterface grbInterface, GRBEnv env, PreProcessBV preProcessBV) {
    ...
    public void addCutUnsatCoreGraeme(HashMap<String, TransformedFormula> transformedFormulas, Context context, String[] conflicts, Solver deltaSolver, Model previousDeltaModel) {
        ...
        for (String coreId : conflicts) {
            ...
            if (tf.isPrecise() || !tf.isDeltaIntroduced()) {
                //System.out.println("gen cons wo d " + tf.getFormula().get(0).getFormula());
                cutSingleConstr = context.mkNot(idStringToBoolExpr.get(id));
                cutExpr = context.mkOr(cutExpr, cutSingleConstr);
            } else {
                index = getConstraintIndex(coreId);
                ArithFormula af = tf.getFormula().get(index);
                //System.out.println("gen cons " + af.getFormula());
                if (af.isEquality()) {
...
//returns assumptions in (previous-current)
    ...
    public GRBModel addAiBjToGurobi(Z3Interface z3Interface, InterpolationContext context, GRBEnv env, GurobiInterface grbInterface, Model modelA, Model modelB, GRBModel grbModelAB) {
        //remove all constraints from grbModelAB and add all constraints representing a_i and b_j  
        removeAllConstraintFromGrbSolver(grbInterface, grbModelAB);
        addConstraintsToGurobi(transformedFormulasA, env, grbInterface, grbModelAB, context, z3Interface, modelA);
        addConstraintsToGurobi(transformedFormulasB, env, grbInterface, grbModelAB, context, z3Interface, modelB);
        return grbModelAB;
    }

    public String[] getUnsatCoreAB(Z3Interface z3Interface, InterpolationContext context, GRBEnv env, GurobiInterface grbInterface, Model modelA, Model modelB, GRBModel grbModelAB) {
        grbModelAB = addAiBjToGurobi(z3Interface, context, env, grbInterface, modelA, modelB, grbModelAB);
        Witness w = solveArithFormula(grbModelAB, context, z3Interface, grbInterface);
        if (w.isSatisfiable()) {
            return null;
        } else {
            return w.getUnsatCore();
        }
    }

    public BoolExpr[] getInterpolantFromCoreAndCutForAandB(Z3Interface z3Interface, InterpolationContext context, GRBEnv env, GurobiInterface grbInterface, Model modelA, Model modelB, GRBModel grbModelAB) {
        String[] core = getUnsatCoreAB(z3Interface, context, env, grbInterface, modelA, modelB, grbModelAB);
        if (core == null) { //it was satisfiable over reals
            return null; //use integer method to compute interpolants
        }
        //there is an unsat core, get A and B

        BoolExpr muA = context.mkTrue();
        BoolExpr muB = context.mkTrue();
        int countA = 0;
        int countB = 0;
        int coreLength = core.length;
        String originalCoreId;
        int formulaASize = transformedFormulasA.size();
        TransformedFormula tf;
        int originalIndex;
        int indexWithinCore;
        BoolExpr trueExpr = context.mkTrue();
        BoolExpr formulaA = trueExpr;
        BoolExpr formulaB = trueExpr;
        BoolExpr af;
        for (int i = 0; i < coreLength; i++) {
            originalCoreId = mapIdToOriginal(context, core[i]);
            originalIndex = Integer.parseInt(originalCoreId.substring(originalCoreId.indexOf("fmla_") + 5));
            //check if originalCoreId is on A or B side
            if (originalIndex < formulaASize) { //formula index starts with 0
                tf = transformedFormulasA.get(originalCoreId);
                indexWithinCore = getConstraintIndex(core[i]);
                af = tf.getFormula().get(indexWithinCore).getFormula();
                af = (BoolExpr) Z3Interface.evalExprInModelWoMc(modelA, af);
                formulaA = context.mkAnd(formulaA, af);
                countA++;
                muA = context.mkAnd(muA, getDeltaValueUnderModelForId(core[i], transformedFormulasA, context, modelA));
            } else {
                tf = transformedFormulasB.get(originalCoreId);
                indexWithinCore = getConstraintIndex(core[i]);
                af = tf.getFormula().get(indexWithinCore).getFormula();
                af = (BoolExpr) Z3Interface.evalExprInModelWoMc(modelB, af);
                formulaB = context.mkAnd(formulaB, af);
                countB++;
                muB = context.mkAnd(muB, getDeltaValueUnderModelForId(core[i], transformedFormulasB, context, modelB));
            }
        }
        BoolExpr notA = context.mkNot(muA);
        BoolExpr notB = context.mkNot(muB);
        if (countA == 0) { //b_j is inconsistent
            return new BoolExpr[]{trueExpr, notA, notB};
        }
        if (countB == 0) { //a_i is inconsistent
            return new BoolExpr[]{context.mkFalse(), notA, notB};
        }
        //compute the interpolant using z3
        return new BoolExpr[]{getInterpolant(formulaA, formulaB, context, z3Interface), notA, notB};

    }

    public void removeAllConstraintFromGrbSolver(GurobiInterface grbInterface, GRBModel grbModel) {
        GRBConstr[] grbConstrs = grbModel.getConstrs();
        int nrConstraints = grbConstrs.length;
        for (int i = 0; i < nrConstraints; i++) {
            grbInterface.removeConstraint(grbModel, grbConstrs[i]);
        }
    }

    public void addConstraintsToGurobi(HashMap<String, TransformedFormula> transformedFormulas,
            GRBEnv env, GurobiInterface grbInterface, GRBModel grbModel, Context context, Z3Interface z3Interface, Model quotientModelA) {
        Set<String> keys = transformedFormulas.keySet();
        for (String id : keys) {
            boolean isTrueFormula = transformedFormulas.get(id).getOriginalFormula().isTrue();
            if (!isTrueFormula) {
                addConstraintToGrbSolver(transformedFormulas, id, grbModel, env, grbInterface, context, z3Interface, quotientModelA, refinementCount);
            }
        }
    }

    //putting Q_a and Q_b together
    public BoolExpr computeInterpolantTransformedFormula(Z3Interface z3Interface, InterpolationContext context, GRBEnv env, GurobiInterface grbInterface, Solver solverA, Solver solverB, GRBModel grbModelA, GRBModel grbModelB, GRBModel grbModelAB) {
        BoolExpr interpolant = context.mkFalse(); //final interpolant formula
        Model modelA;
        BoolExpr partialInterpolant = context.mkTrue();
        //solverA= solverA union solverB
        solverA = z3Interface.copySolverFromTo(solverB, solverA, context);
        while (z3Interface.checkFormula(solverA) == Status.SATISFIABLE) { //external loop, corresponding to formula A
            ...
            //System.out.println("outside " + modelA);
            //the following notMuA is to use later, so that we do not need to recompute within the inner loop
            BoolExpr notMuA = blockPreviousModel(transformedFormulasA, context, solverA, modelA);
            //removeConstraintAlreadyInGrbSolver(grbInterface, grbModelA, refinementCount, previousAssumptionsA, currentAssumptionsA);

            BoolExpr formulaUnderCurrentAssA = addConstraintsToGurobiAndReturnAddedFormulas(transformedFormulasA, env, grbInterface, grbModelA, context, z3Interface, modelA);
            //System.out.println("formula " + formulaUnderCurrentAssA);
            ...
            if (!LIAWitnessA.isSatisfiable()) {
                ...
                Solver solverbInnerLoop = z3Interface.getSolverClone(solverA, context); //now sovlerA
...
//                printAssertion(solverB);
                while (z3Interface.checkFormula(solverbInnerLoop) == Status.SATISFIABLE) { //external loop, corresponding to formula A
                    ...
                    BoolExpr formulaUnderCurrentAssB = addConstraintsToGurobiAndReturnAddedFormulas(transformedFormulasB, env, grbInterface, grbModelB, context, z3Interface, modelB);
//                    System.out.println("B formula " + formulaUnderCurrentAssB);
                    ...
                    if (!LIAWitnessB.isSatisfiable()) {
                        ...
                        //both a_i and b_j are satisfiable, but not their conjunction
                        //collect a_i and b_j and generate unsat_core
                        BoolExpr[] partialInterpolantAndCut = getInterpolantFromCoreAndCutForAandB(z3Interface, context, env, grbInterface, modelA, modelB, grbModelAB);
                        if (partialInterpolantAndCut == null) { //just to account for the fact that Gurobi may return satisfiable over reals even though it is unsat over integer
                            partialInterpolant = getInterpolant(formulaUnderCurrentAssA, formulaUnderCurrentAssB, context, z3Interface); //integer computation
                            if (partialInterpolant == null) { // this case as returned by z3
                                System.err.println("The constraints are satisfiable: so no interpolant.");
                                System.exit(1);
                            }
                            //since partialInterpolant is conjoined with interpolantConj, we cannot get more stronger than this than this
                            if (partialInterpolant.isFalse()) {
                                interpolantConj = partialInterpolant;
                                break; //breaks out of the inner loop
                            }
                            interpolantConj = context.mkAnd(interpolantConj, partialInterpolant);
                            BoolExpr notMuB = blockPreviousModel(transformedFormulasB, context, solverbInnerLoop, modelB);
                            solverbInnerLoop.add(notMuB); // not the current mu_b 
                            //solverA.add(context.mkOr(notMuA, notMuB)); // not the conjunction mu_a and mu_b 
                            solverA.add(notMuA);
                        } else {
                            //partialInterpolantAndCut={partialInterpolant, notA, notB}
                            interpolantConj = context.mkAnd(interpolantConj, partialInterpolantAndCut[0]);
                            solverbInnerLoop.add(partialInterpolantAndCut[2]); // not the current mu_b 
                            //System.out.println("notB " + partialInterpolantAndCut[2]);
                            //notMuA = partialInterpolantAndCut[1];
                            solverA.add(partialInterpolantAndCut[1]);
                            //solverB.add(context.mkOr(partialInterpolantAndCut[1], partialInterpolantAndCut[2]));
                            //solverA.add(context.mkOr(partialInterpolantAndCut[1], partialInterpolantAndCut[2])); // not the conjunction mu_a and mu_b  
                        }
                    }
                }
    ...
    //putting Q_a and Q_b separately
//    public BoolExpr computeInterpolantTransformedFormula(Z3Interface z3Interface, InterpolationContext context, GRBEnv env, GurobiInterface grbInterface, Solver solverA, Solver solverB, GRBModel grbModelA, GRBModel grbModelB, GRBModel grbModelAB) {
//        BoolExpr interpolant = context.mkFalse(); //final interpolant formula
//        Model modelA;
//        //Solver solverB1= z3Interface.getSolverClone(solverB, context); 
//        BoolExpr partialInterpolant = context.mkTrue();
//        //solverA= solverA union solverB
//        //solverA = z3Interface.copySolverFromTo(solverB, solverA, context);
//        while (z3Interface.checkFormula(solverA) == Status.SATISFIABLE) { //external loop, corresponding to formula A
...
//                //partial interpolant of a_i with B
//                BoolExpr interpolantConj = context.mkTrue();
//                Solver solverbInnerLoop = z3Interface.getSolverClone(solverB, context); //now sovlerA
////                System.out.println("beginning");
////                printAssertion(solverB);
//                while (z3Interface.checkFormula(solverbInnerLoop) == Status.SATISFIABLE) { //external loop, corresponding to formula A
//                    Model modelB = solverbInnerLoop.getModel();
...
//                            solverbInnerLoop.add(partialInterpolantAndCut[2]); // not the current mu_b 
//                            notMuA= partialInterpolantAndCut[1];
//                            //solverA.add(context.mkOr(notMuA, partialInterpolantAndCut[2])); // not the conjunction mu_a and mu_b  
//                        }
//                    }
//                }
//                //since interpolantConj is disjoined with interpolant, we cannot get more general than this
...
//    }
    ...
    public BoolExpr addConstraintsToGurobiAndReturnAddedFormulas(HashMap<String, TransformedFormula> transformedFormulas,
        ...
        for (String id : keys) {
            boolean isTrueFormula = transformedFormulas.get(id).getOriginalFormula().isTrue();
            if (!isTrueFormula) {
                z3Formula = addConstraintToGrbSolverConditional(transformedFormulas, id, grbModel, env, grbInterface, context, z3Interface, quotientModelA, refinementCount);
    ...
    public BoolExpr addConstraintToGrbSolverConditional(HashMap<String, TransformedFormula> transformedFormulas, String formulaId, GRBModel grbModel, GRBEnv env, GurobiInterface grbInterface, Context context, Z3Interface z3Interface, Model deltaModel, int refinementCount) {
        ...
        if (alreadyInSolver) {
            ...
            if (tf.isDeltaIntroduced()) {
                //System.out.println("removed "+formulaId);
                for (int i = 0; i < nrConstraints; i++) {
                    String idNew = formulaId + "__r__" + i;
                    grbInterface.removeConstraint(grbModel, idNew);
                }
                //add the fresh ones except those precise, precise ones do  not changed and are not eliminated
                //System.out.println("added   "+formulaId);
                ...
                //only to return as z3 formula later, do not add this to the solver
                simplifiedFormula = extractBoolFormula(tf.getFormula(), context, deltaModel);
            }

        } else { //not in the solver
//            System.out.println("added   "+formulaId);
            if (!tf.isDeltaIntroduced()) {
                simplifiedFormula = extractBoolFormula(tf.getFormula(), context, deltaModel);
            } else {
    ...
    public void addConstraintToGrbSolver(HashMap<String, TransformedFormula> transformedFormulas, String formulaId, GRBModel grbModel, GRBEnv env, GurobiInterface grbInterface, Context context, Z3Interface z3Interface, Model deltaModel, int refinementCount) {
        ArrayList<BoolExpr> simplifiedFormula = null;
        TransformedFormula tf = transformedFormulas.get(formulaId);
        if (!tf.isDeltaIntroduced()) {
            simplifiedFormula = extractBoolFormula(tf.getFormula(), context, deltaModel);
    ...
    public BoolExpr blockPreviousModel(HashMap<String, TransformedFormula> transformedFormulas, Context ctx, Solver solver, Model model) {
        BoolExpr acc = ctx.mkTrue();
        TransformedFormula tf;
        ArithExpr deltaExpr;
        String s;
    ...
    boolean isIntExprAConstant(IntExpr ie) {
        try {
            Long.parseLong(ie.toString());
            return true;
    ...
    public BoolExpr getDeltaValueUnderModelForId(String s1, HashMap<String, TransformedFormula> transformedFormulas, Context ctx, Model model) {
        TransformedFormula tf;
        ArithExpr deltaExpr;
        String s;
        s = mapIdToOriginal(ctx, s1);
        int index = Integer.parseInt(s1.substring(s1.indexOf("__r__") + 5));
        tf = transformedFormulas.get(s);
        if (tf.deltaIntroduced) {
            deltaExpr = tf.formula.get(index).deltaExpr;
            //there are two choices whether deltaExpr is sub/unary minus or a single int expr
            if (deltaExpr.isConst()) {
                return ctx.mkEq(deltaExpr, Z3Interface.evalExprInModel(model, deltaExpr));
            }
            if (deltaExpr.isSub()) {
                Expr[] exprs = deltaExpr.getArgs();
                if (!isIntExprAConstant((IntExpr) exprs[0]) && !isIntExprAConstant((IntExpr) exprs[1])) {
                    return ctx.mkAnd(ctx.mkEq(exprs[0], Z3Interface.evalExprInModel(model, exprs[0])), ctx.mkEq(exprs[1], Z3Interface.evalExprInModel(model, exprs[1])));
                }
                if (!isIntExprAConstant((IntExpr) exprs[0])) {
                    return ctx.mkEq(exprs[0], Z3Interface.evalExprInModel(model, exprs[0]));
                }
                if (!isIntExprAConstant((IntExpr) exprs[1])) {
                    return ctx.mkEq(exprs[1], Z3Interface.evalExprInModel(model, exprs[1]));
                }
            }
            //case -deltaexpr (not a delta var or a sub)
            return ctx.mkEq(deltaExpr, Z3Interface.evalExprInModel(model, deltaExpr));

        } else if (tf.isPrecise() || !tf.isDeltaIntroduced()) {
            return idStringToBoolExpr.get(s);
        }
        return null;
    }
...
//            Util.print(PRINT_FLAG, "finished identifying");
            ...
            GRBModel grbModelAB = grbInterface.getGRBModel(env);
            ...
            Util.createZ3GRBVarMap(grbModelAB, vars, z3GRBVarMap, context);
            grbInterface.updateModel(grbModelAB); // update gurobi model after variable addition
            ...
            BoolExpr interpolantFormula = (BoolExpr) computeInterpolantTransformedFormula(z3Interface, context, env, grbInterface, solverA, solverB, grbModelA, grbModelB, grbModelAB).simplify();
            z3Interface = null;
...
//        }
    ...
    public BoolExpr interpolantMAFromFile(String inputFile) {
        ...
        //BoolExpr interpolantBV= getInterpolant(formulaA, formulaB, context, z3Interface);
        //System.out.println("interpolant bv "+interpolantBV);
...
//            System.out.println("arith for "+arithFormulas.get(0).formula);
    ...
    public void printArithFormula(ArithFormula ariths) {
        // System.out.println("a "+ariths.);
    }
    ...
    public TransformedFormula getTransFormulaCorrespondingToBooleanConstants(GRBModel grbModel, Context context, TransformedFormula tf, Z3Interface z3Interface,
            GurobiInterface grbInterface, GRBEnv env) {
...
//            System.out.println("preprocessed expr ================= "+ preProcessedExpr);
        if (preProcessedExpr.isFalse()) {
            ...
            deltaSolver.add(deltaBounds1);
            deltaSolver.add(deltaBounds2);
        }
...
//        }
    ...
    public TransformedFormula getTransformedFormula(HashMap<String, TransformedFormula> transformedFormulas, GRBModel grbModel, Context context, String id, Z3Interface z3Interface, GurobiInterface grbInterface, GRBEnv env, Solver deltaSolver) {
        ...
        if (formula.isTrue() || formula.isFalse()) {
            return getTransFormulaCorrespondingToBooleanConstants(grbModel, context, tf, z3Interface, grbInterface, env);
        }
        ArrayList<ArithFormula> arithFormulas = new ArrayList<>();
        PreProcessBV ppbv = new PreProcessBV();
        BoolExpr maExpr = genAinEqFromAtomicBvinEq(context, id, formula, z3Interface, grbModel, grbInterface, env, ppbv);
...
//        }
    ...
    public BoolExpr simplifyMultBy1BVAtomicFormula(BoolExpr e, Context ctx) {
        Expr[] exprs = e.getArgs();
        BoolExpr be = null;
        //System.out.println("exprs[0]" + exprs[0]);
    ...
    public BitVecExpr simplifyMultBy1BVExpr(BitVecExpr bvExpr, Context ctx) {
        ...
        //System.out.println("size " + bvSize);
        BitVecExpr one = ctx.mkBV(1, bvSize);
        //System.out.println("bv expr " + bvExpr);
        //the following if condition is introduced since Z3 4.5.1 produced error in bvmul, earlier version had no problem
        if (bvExpr.isConst()) {
            return bvExpr;
        }
        if (bvExpr.isBVMul()) {
...
//                System.out.println("calling collect constraints");
    ...
    public BoolExpr computeMAInterpolant(String file) {
        ...
        try {
            ...
            System.out.println(Message.showQEStatistics(file, time / 1000000, formulaCount, refinementCount, nrConstraintRelaxed));
            dispose();
