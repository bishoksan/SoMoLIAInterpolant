 ...
 * @author kafle
 */
public class InterpolantBendersDecompWFixedSizeLazyWoPP {
...
//returns assumptions in (previous-current)
    ...
    public BoolExpr computeInterpolantTransformedFormula(Z3Interface z3Interface, InterpolationContext context, GRBEnv env, GurobiInterface grbInterface, Solver solverA, GRBModel grbModelA, GRBModel grbModelB) {
...
//            return null; //all delta coeff are tried, and find a different disjunct, 
            ...
            if (!LIAWitnessA.isSatisfiable()) {
                ...
                Solver solverB = z3Interface.getSolverClone(solverA, context);
//                System.out.println("beginning");
//                printAssertion(solverB);
                while (z3Interface.checkFormula(solverB) == Status.SATISFIABLE) { //external loop, corresponding to formula A
                    Model modelB = solverB.getModel();
...
//                    System.out.println("B formula " + formulaUnderCurrentAssB);
                    ...
                    if (!LIAWitnessB.isSatisfiable()) {
                        updateModelWithCutFromCore(transformedFormulasB, LIAWitnessB, context, solverB, modelB);
                        //System.out.println("core ");
                        //printAssertion(solverB);
                        //even for solver A, use the modelB: eliminate that quotient values from sover A
                        addCutUnsatCoreGraeme(transformedFormulasB, context, LIAWitnessB.getUnsatCore(), solverA, modelB);
                        //refinementCount++;
                    } else {
                        ...
                        BoolExpr notMuB = blockPreviousModel(transformedFormulasB, context, solverB, modelB, currentAssumptionsB, cutAssumptionsB);
                        solverB.add(notMuB); // not the current mu_b
...
//TODO: remove quantified variables which are not in the formula
    ...
    public BoolExpr interpolantMAFromFormula(BoolExpr formulaA, BoolExpr formulaB, InterpolationContext context) {
        try {
            ...
            BoolExpr boolStructureB = assignIdToBVFormula(formulaB, context, transformedFormulasB, solverA);//we put all into the solver A and later copy to solver B
//            Util.print(PRINT_FLAG, "finished identifying");
            ...
            tranformAndStoreFormulas(transformedFormulasB, grbModelB, context, z3Interface, grbInterface, env, solverA); //solverA since we collect quotients in the same solver
            BoolExpr interpolantFormula = (BoolExpr) computeInterpolantTransformedFormula(z3Interface, context, env, grbInterface, solverA, grbModelA, grbModelB).simplify();
            z3Interface = null;
    ...
    public BoolExpr getInterpolantByPreProcessingFormulas(BoolExpr formulaA, BoolExpr formulaB, Context ctx, Z3Interface z3Interface) {
        ...
        if (formulaA.isTrue()) {
            if (formulaB.isFalse()) {
                return ctx.mkTrue();
            } else { //the formula together can be satisfiable, since we could not prove the unsat of ppFormulaB
                return null;
            }
        }
        if (formulaB.isFalse()) {
